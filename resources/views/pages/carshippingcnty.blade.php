...
@section('content')
    <div class="section dark mb-0"
        ...
        <div class="container">
            <h2>Car Shipping in {{$stslug}}, {{ucfirst(strtolower($cntyslug))}} County</h2>
                    ...
                    <ul class="nav nav-tabs nav-fill flex-column border-bottom-0 flex-md-row bg-color mt-4" role="tablist"
                        ...
                        <li class="nav-item @if ($errors->count() == 0) active @endif @error('movingfromzip1') active @enderror @error('movingtozip1') active @enderror @error('movingdate1') active @enderror @error('movesize1') active @enderror"
                            role="tabpanel"><a
                                ...
                                href="#carshipping" aria-controls="carshipping" role="tab"
                                data-toggle="tab">carshipping</a></li>
                    </ul>
    ...
    <div class="d-flex justify-content-center">
        <ul class="iconlist m-0 pe-5" id="zpcd">
            @foreach ($ctys as $cty)
            <li><a href="{{ url('carshipping') }}/{{$cty->state_code}}/{{$cty->county}}/{{$cty->city}}" title="{{$cty->city}} Car Shipping">Car Shipping {{ ucfirst(strtolower($cty->city)) }}</a></li>
            @endforeach
        </ul>
    </div>
